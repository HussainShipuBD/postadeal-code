	...
	checkHeaderPosition() {
		...
		if($(window).scrollTop() > headerHeight) {
			...
			if($(window).width() < 992) {
				...
				$('.sn-bar').css({'position' : 'fixed','top' : '0px', 'z-index' : '-1' });
				...
				$('#overallscroll_div .flaticon-right-arrow-1.slick-arrow').show();
		     	...
		     	$('.sn-bar').css({'position' : 'relative', 'bottom' : '26px', });
				}
			...
			$('.sn-bar').css({'position' : 'relative','bottom' : '0px', 'z-index' : '1' });
		

			}
			else{

			$('.sn-bar').css({'position' : 'fixed','bottom' : 'unset', });
				}
